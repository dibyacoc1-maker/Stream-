...
function renderSection(title, channels) {
    let cards = channels.map(ch => generateCard(ch)).join('');
...
function renderHome() {
    ...
    // All Channels Section with Category Filters
    const groups = ['All', ...new Set(allChannels.map(ch => ch.group))];
    
    html += `<div class="section" id="all-channels-section">`;
    html += `<h2 class="section-title">All Channels</h2>`;
    html += `<div class="category-filters">`;
    groups.forEach((g, i) => {
        html += `<button class="filter-pill focusable ${i === 0 ? 'active' : ''}" data-group="${g}" tabindex="0">${g}</button>`;
    });
    html += `</div>`;
    html += `<div class="grid" id="all-channels-grid">`;
    html += allChannels.map(ch => generateCard(ch)).join('');
    html += `</div></div>`;
...
function attachFilterListeners() {
    document.querySelectorAll('.filter-pill').forEach(pill => {
        pill.addEventListener('click', () => {
            ...
            const filtered = group === 'All' ? allChannels : allChannels.filter(ch => ch.group === group);
            
            const grid = document.getElementById('all-channels-grid');
            grid.innerHTML = filtered.map(ch => generateCard(ch)).join('');
            attachCardListeners(); // Re-attach listeners for new cards
        });
    });
}
